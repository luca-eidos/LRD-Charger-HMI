...
const ImageGenOverlay: React.FC<ImageGenOverlayProps> = ({ onClose, onImageGenerated, isLightTheme = false, t }) => {
  ...
  return (
    <div className={`absolute inset-0 z-50 backdrop-blur-2xl p-12 flex animate-in slide-in-from-bottom duration-500 ${overlayBg}`}>
      ...
      <div className={`w-[50%] flex flex-col pr-12 border-r transition-colors ${divider}`}>
        <div className="mb-10">
          <h2 className="text-[#00e5ff] text-4xl font-black uppercase italic tracking-tighter">{t('aiThemeEngine')}</h2>
        </div>

        <div className="flex-1 space-y-12">
          <div className="space-y-4">
            <label className={`text-xs font-black uppercase tracking-[0.4em] transition-colors ${textSecondary}`}>{t('visualAtmosphere')}</label>
            <textarea 
              value={prompt}
          ...
          <div className="space-y-4">
            <label className={`text-xs font-black uppercase tracking-[0.4em] transition-colors ${textSecondary}`}>{t('renderResolution')}</label>
      ...
      <div className="flex-1 flex flex-col pl-12 justify-center">
        <div className="text-center mb-12">
          <p className={`text-xl font-bold leading-relaxed mb-6 transition-colors ${isLightTheme ? 'text-slate-600' : 'text-white/90'}`}>
            {t('imageGenDescription')}
          </p>
          <div className="inline-flex items-center gap-3 px-6 py-2 bg-[#00e5ff]/10 rounded-full border-2 border-[#00e5ff]/40">
            <span className="text-[#00e5ff] text-xs font-black uppercase tracking-widest">{t('premiumCustomization')}</span>
          </div>
        </div>
        ...
        <div className={`mt-auto text-center pt-8 border-t transition-colors ${divider}`}>
           <p className={`text-[10px] font-black uppercase tracking-[0.4em] transition-colors ${isLightTheme ? 'text-slate-300' : 'text-white/30'}`}>{t('integratedEngine')}</p>
        </div>
      </div>
    </div>
  );
